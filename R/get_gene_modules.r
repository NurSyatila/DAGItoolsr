...
get_gene_modules <- function(geneNetwork,summaryFile){
  ...
  for (x in seq_along(df_levels)){
	edges <- igraph::gsize(igraph::induced_subgraph(g_sub,unlist(c_sub[x])))
  ...
  if (nrow(dfClusters) > 3) {
    par(mfrow=c(2,2))
    for(i in seq(1:4)){
      suppressMessages(string_db$plot_network(unlist(stringr::str_split(dfClusters$genes[i],",")), add_link=FALSE, add_summary=TRUE))
    }
  }
  else {
    par(mfrow=c(2,2))
